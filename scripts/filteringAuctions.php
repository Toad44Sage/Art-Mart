...
if (isset($_POST['limit']) and (isset($_POST['author']) or isset($_POST['painting']) or isset($_POST['styles']) or isset($_POST['bet_down']) or isset($_POST['bet_up']))) {
    ...
    if (isset($_POST['bet_down']) and isset($_POST['bet_up'])) {
        ...
        $response['data']['current_auctions'] = $mysqli->query("SELECT auctions.auction_id, auctions.painting_id, paintings.name as name, auctions.user_id as author_id, users.login as author, paintings.path_to_paint, styles.name as style, start_date, end_date, start_price, buyout_price, IFNULL(bets.bet, 0) as bet
				FROM auctions
