$(function () {
  ...
  $(".tabs").on("click", ".tab-item", function () {
    ...
    // Проверка на то, что выбран таб "BOUGHT"
    if ($(this).find("span").text() == "BOUGHT") {
      $(".Bought").show(); // делаем видимым класс "Bought"
      $(".Bought").empty(); // очистка галереи от ранее загруженных изображений
      offset = 0; // сброс значения переменной offset
      loadPostsBought(); // загрузка постов
    } else {
      $(".Bought").hide(); // скрываем класс "Bought"
    }
  });
...
$(document).ready(function () {
  ...
  $("#followers-list").on("click", "li", function () {
    ...
    window.location.href =
      "../../profile/HTML/profile.html?id=" + clickedUserId;
  });
});
...
$(document).ready(function () {
        ...
        success: function (response) {
          if (response.success == "1") {
            ...
            $.each(response.data, function (index, follower) {
              ...
              listItem.attr("data-user-id", follower.following_id);
              followersList.append(listItem);
            });
  ...
  // id получаю по клику на элемент
  $("#following-list").on("click", "li", function () {
    let clickedUserId = $(this).attr("data-user-id");
    window.location.href =
      "../../profile/HTML/profile.html?id=" + clickedUserId;
  });
